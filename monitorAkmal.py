...
RE_SUCCESS = re.compile(r"Accepted\s+(password|publickey|keyboard-interactive(?:/pam)?)\s+for\s+(\S+)\s+from\s+(\S+)")
...
SQLI_PATTERNS = [r"(?i)(union(\s+all)?\s+select)", r"(?i)(or\s+1=1)", r"(?i)(' or '1'='1)", r"(?i)(sleep\()", r"(?i)(information_schema)"]
    ...
    for target in FONNTE_TARGETS:
        try:
            ...
            if resp.status_code == 200:
                ...
                logging.warning(f"[WA ERROR] {resp.text}")
...
🚨 DETEKSI  : {', '.join(detection) if detection else '-'}
...
def detect_patterns(req):
    ...
    for p in SQLI_PATTERNS:
        if re.search(p, req): found.append("SQLi")
    for p in XSS_PATTERNS:
        if re.search(p, req): found.append("XSS")
    return found
...
def main():
    ...
    while True:
        for p, f in files.items():
            ...
            # SSH Brute-force
            if evt["type"] == "login_fail" and not is_admin_ip(ip):
                dq = ip_fail[ip]
                dq.append(time.time())
            ...
            # Web attacks
            elif evt["type"] == "web_access" and not is_admin_ip(ip):
                req = evt["req"]
                patterns = detect_patterns(req)
                if patterns or evt["status"] >= 400:
                    summary = f"Permintaan mencurigakan dari {ip}: {req} (status {evt['status']})"
